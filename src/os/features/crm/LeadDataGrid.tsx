...
export function LeadDataGrid({ leads, onReload, onSelectLead, initialFilter }: LeadDataGridProps) {
  ...
  const hasActivePopover = Boolean(activeStatusPopover || activePriorityPopover);
  ...
  const renderRow = (lead: LeadItem) => {
    ...
    return (
      <div
        ...
        className={cn(
          "grid grid-cols-[36px_100px_1.5fr_1fr_1.1fr_100px_120px_90px] gap-3 p-3.5 items-center group transition-colors duration-150 hover:bg-slate-50/90 cursor-pointer text-xs relative",
      ...
      >
        ...
        <div className="flex flex-col truncate">
          ...
          <span className="text-[11px] text-slate-400 font-mono">{lead.client_phone}</span>
        </div>
        ...
        <div className="flex flex-col truncate">
          ...
          <span className="text-[11px] text-slate-400 mt-0.5">{lead.move_date || 'Date à fixer'}</span>
        </div>
          ...
          <AnimatePresence>
            {activePriorityPopover === lead.id && (
              <motion.div
                ...
                className="absolute top-full left-0 mt-1.5 w-36 bg-white rounded-2xl shadow-[0_12px_36px_-4px_rgba(0,0,0,0.14),0_4px_12px_rgba(0,0,0,0.06)] border border-slate-200/90 p-1.5 z-50 ring-1 ring-black/5"
              >
        ...
        <div className="flex flex-col gap-1" onClick={e => e.stopPropagation()}>
          ...
          <div className="relative">
            <button
              ...
              className={cn(
                "flex items-center justify-between gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold border transition-transform active:scale-95 shadow-sm",
                statusMeta.bg,
                statusMeta.text
              )}
            >
            ...
            <AnimatePresence>
              {activeStatusPopover === lead.id && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0, scale: 1 }}
                  exit={{ opacity: 0, y: 4, scale: 0.95 }}
                  className="absolute top-full left-0 mt-1.5 w-48 bg-white rounded-2xl shadow-[0_12px_36px_-4px_rgba(0,0,0,0.14),0_4px_12px_rgba(0,0,0,0.06)] border border-slate-200/90 p-1.5 z-50 ring-1 ring-black/5"
                >
                  {ALL_STATUSES.map(st => (
                    <button
                      key={st}
  ...
  return (
    <div className="space-y-4 relative">
      {/* Global Transparent Click-Away Backdrop for Popovers */}
      {hasActivePopover && (
        <div 
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-white p-3 rounded-2xl border border-slate-200/80 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_1px_2px_rgba(0,0,0,0.02)]">
        {/* Search Input */}
        <div className="relative flex-1 max-w-md">
          ...
          <input 
            ...
            className="w-full pl-9 pr-8 py-1.5 rounded-xl border border-slate-200 text-xs text-slate-800 focus:border-slate-900 focus:outline-none placeholder:text-slate-400"
          />
          {searchQuery && (
        ...
        <div className="flex items-center justify-between gap-2 overflow-x-auto">
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className={cn(
                "px-2.5 py-1 rounded-lg text-xs font-semibold transition-colors",
                selectedStatusTab === 'nouveau' ? "bg-blue-600 text-white" : "text-slate-500 hover:bg-slate-100"
              )}
            >
      ...
      {viewMode === 'kanban' ? (
          ...
          {groupedData.map(group => {
            ...
            return (
              <div 
                key={group.id} 
                className={cn(
                  "bg-white rounded-2xl border border-slate-200/80 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_1px_2px_rgba(0,0,0,0.02)] border-l-4 transition-shadow hover:shadow-[0_4px_20px_-4px_rgba(0,0,0,0.06)]", 
                  group.color
                )}
              >
                ...
                <div 
                  ...
                  className="p-3.5 bg-slate-50/70 border-b border-slate-200/60 rounded-t-2xl flex items-center justify-between cursor-pointer hover:bg-slate-100/70 transition-colors"
                >
        ...
        <div className="w-full bg-white rounded-2xl border border-slate-200/80 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_1px_2px_rgba(0,0,0,0.02)] flex flex-col">
          {/* Header Row */}
          <div className="grid grid-cols-[36px_100px_1.5fr_1fr_1.1fr_100px_120px_90px] gap-3 p-3.5 border-b border-slate-200/70 bg-slate-50/70 text-[11px] font-bold text-slate-400 uppercase tracking-wider items-center rounded-t-2xl">
      ...
      <AnimatePresence>
        {selectedLeads.size > 0 && (
          <motion.div
            ...
            exit={{ y: 50, opacity: 0 }}
            className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-5 py-2.5 rounded-2xl shadow-2xl flex items-center gap-4 z-[100] border border-white/10"
          >
